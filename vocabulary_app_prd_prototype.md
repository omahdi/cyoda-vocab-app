...
- **NFR-1 Performance:** First paint <2s on broadband; table ops <200ms for common actions; handle 10k rows.
- **NFR-2 Reliability:** No data loss on import/export; server crash tolerance (durable storage).
- **NFR-3 Security:** Tokens must be unguessable; rate-limit public endpoints; CORS limited; CSRF not applicable to read-only views.
- **NFR-4 Privacy:** No PII expected; no third-party trackers; opt-out of analytics by default.
- **NFR-5 Accessibility:** Keyboard navigation for table/edit; focus states; sufficient contrast; ARIA where appropriate.
- **NFR-6 Internationalization:** Full UTF‑8 support; right‑to‑left scripts display correctly.
...
## 8) API Endpoints (suggested for prototype)
...
## 11) Telemetry (opt-in for dev builds)
- Count imports, exports, link creations, link opens.
- Store last error type for import failures (aggregated).
...
## 14) Technical Notes & Constraints (non-binding)
- **Frontend:** React/TypeScript; virtualized table (e.g., React-Window) for performance.
- **Backend:** Lightweight Node/Express or Python/FastAPI; CSV streaming parser.
- **Storage:** SQLite (file) or lightweight Postgres for durability; migrations via Prisma/SQLAlchemy.
- **Security:** Token links only; disable indexing; add simple rate limiting on `/s/:token`.
...
- **IDs supplied by CSV:** Accept external ids for new items (current spec: yes); risk of collision/typo.
- **Large datasets:** Performance beyond 50k rows (out of current scope) may need server-side pagination.
- **Lesson semantics:** Free-form text vs. constrained set; current spec uses free-form text.
- **Export formatting:** Default BOM/line endings—make configurable at export time.
- **Public link scope:** For now, links expose the full dataset; future: per-lesson or time-bounded links.
